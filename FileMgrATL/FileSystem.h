...
namespace FileSystem
{
	...
	public:
		...
		std::string getTopDrive(std::string file);
		void FileSearch_all(std::vector<std::string> &files, std::string path, std::vector<std::string> patterns);
		void find(std::vector<std::string>& filesToProcess_, const std::string & path, const std::vector<std::string> patterns);
		std::vector<int> Findmatchinglist(std::string sReg, std::string filePath);
		void getAll(std::vector<std::string> &files, std::string path, std::vector<std::string> patterns);
		void close();
	private:
		HANDLE hFindFile;

...
std::vector<std::string> Directory::getDirectories(const std::string& path, const std::string& pattern)
{
	...
	if (main.empty())
	{
		...
		if (dir != "Debug" && dir != ".."  && dir != "." && dir != "Release")
		{
			main = Path::fileSpec(path, dir);
			dirs.push_back(main);
		}
		while (true)
		{
...
void FileSystemSearch::FileSearch_all(std::vector<std::string> &list, std::string dest, std::vector<std::string> exts)
{
	std::vector<std::string> getallDirs = Directory::getDirectories((dest));
	std::vector<std::string> getallFil = Directory::getFiles((dest));

	if (getallFil.empty() && getallDirs.empty())
		return;
	for (auto a : getallFil)
	{
		for (auto b : exts)
		{
			if (b == (Path::getExt(Path::getFullFileSpec(a))))
			{
				std::string path1 = Path::getPath(a);
				std::string name = Path::getName(a);
				std::string fullname = path1 + name;
	...
	for (auto c : getallDirs)
	{
		FileSearch_all(list, c, exts);
	}

	if (list.empty() && getallDirs.empty())
		return;
}

void FileSystemSearch::find(std::vector<std::string> &filesToProcess_, const std::string& path, const std::vector<std::string> patterns)
{
	...
	for (auto d : dirs)
	{
		...
		find(filesToProcess_,dpath,patterns);   // recurse into subdirectories
...
int main(int argc, char* argv[])
{
	...
	std::vector<std::string> currdirs1 = Directory::getDirectories(path);
